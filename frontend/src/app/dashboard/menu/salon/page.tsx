...
import {
  Scissors, Plus, Edit, Trash2, Search, X, Check, ToggleLeft, ToggleRight
} from 'lucide-react';
...
const SALON_CATEGORIES = [
  'HAIRCUT', 'STYLING', 'FACIAL', 'MANICURE', 'PEDICURE',
  'MASSAGE', 'TREATMENT', 'BODY', 'BRIDAL', 'GROOMING', 'OTHER',
];

const emptyForm = { name: '', category: 'HAIRCUT', price: '', department: 'SALON', isAvailable: true };
...
export default function SalonMenuPage() {
  ...
  const [editing, setEditing] = useState<SalonItem | null>(null);
  const [form, setForm] = useState(emptyForm);
  const [saving, setSaving] = useState(false);
  const canCreateMenu = usePermission('salon-menu', 'create');
  const canUpdateMenu = usePermission('salon-menu', 'update');
  const canDeleteMenu = usePermission('salon-menu', 'delete');

  const fetchItems = useCallback(async () => {
    try {
      const res = await api.get('menu', { params: { department: 'SALON' } });
      setItems(res.data);
  ...
  const openEdit = (item: SalonItem) => {
    setEditing(item);
    setForm({ name: item.name, category: item.category, price: String(item.price), department: 'SALON', isAvailable: item.isAvailable });
    setShowModal(true);
  };
  ...
  const toggleAvailability = async (item: SalonItem) => {
    try {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-slate/60 font-semibold mt-1 text-sm">Manage salon & spa service catalogue — no GST applied</p>
          </div>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-5 py-3 flex items-center gap-3">
          ...
          <span className="text-xs font-semibold text-emerald-800">
            Salon services are GST-exempt. Prices shown are final payable amounts.
          </span>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-navy/[0.02] border-b border-navy/[0.05]">
                <th className="px-8 py-5 text-[9px] font-black text-slate uppercase tracking-[0.3em]">Service</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg mx-4 p-8">
            ...
            <form onSubmit={handleSave} className="space-y-5">
              <div>
                <label className="block text-[10px] font-black text-navy/50 uppercase tracking-[0.2em] mb-1.5">Service Name</label>
                <input
                  type="text" value={form.name}
                  onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                  className={inputClass} placeholder="e.g. Classic Haircut" required
                />
              </div>
              <div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <span className="text-xs font-bold text-navy/60">
                  {form.isAvailable ? 'Available for booking' : 'Hidden from booking'}
                </span>
              </div>
              <div className="flex gap-3 pt-2">
                ...
                >{saving ? 'Saving...' : editing ? 'Update Service' : 'Add Service'}</button>
              </div>
            </form>
          </div>
        </div>
