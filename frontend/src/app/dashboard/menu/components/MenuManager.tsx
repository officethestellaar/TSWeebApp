...
export default function MenuManager({ config, screenKey }: { config: MenuConfig; screenKey: string }) {
  const canCreate = usePermission(screenKey, 'create');
  const canUpdate = usePermission(screenKey, 'update');
  const canDelete = usePermission(screenKey, 'delete');
  const [items, setItems] = useState<MenuItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [editing, setEditing] = useState<MenuItem | null>(null);
  const [form, setForm] = useState({ name: '', category: config.categories[0], price: '', department: config.department, isAvailable: true });
  const [saving, setSaving] = useState(false);
  const Icon = config.icon;

  const fetchItems = useCallback(async () => {
    try {
      const res = await api.get('menu', { params: { department: config.department } });
      setItems(res.data);
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ name: '', category: config.categories[0], price: '', department: config.department, isAvailable: true });
    setShowModal(true);
  };

  const openEdit = (item: MenuItem) => {
    setEditing(item);
    setForm({ name: item.name, category: item.category, price: String(item.price), department: config.department, isAvailable: item.isAvailable });
    setShowModal(true);
  };
    ...
    <div className="p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-slate/60 font-semibold mt-1 text-sm">
              Manage {config.title.toLowerCase()} catalogue — {config.gstRate}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <ExportButton
              filename={`${config.department.toLowerCase()}-menu`}
            ...
            {canCreate && (
              ...
              >
                <Plus size={14} /> Add {config.department === 'GYM' || config.department === 'PERSONAL_TRAINER' ? 'Service' : 'Item'}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-3 flex items-center gap-3">
          ...
          <span className="text-xs font-semibold text-amber-800">{config.gstNote}</span>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-navy/[0.02] border-b border-navy/[0.05]">
                <th className="px-8 py-5 text-[9px] font-black text-slate uppercase tracking-[0.3em]">Name</th>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg mx-4 p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-navy flex items-center gap-2">
                ...
                {editing ? `Edit ${config.department === 'GYM' || config.department === 'PERSONAL_TRAINER' ? 'Service' : 'Item'}` : `Add ${config.department === 'GYM' || config.department === 'PERSONAL_TRAINER' ? 'Service' : 'Item'}`}
              </h2>
            ...
            <form onSubmit={handleSave} className="space-y-5">
              <div>
                <label className="block text-[10px] font-black text-navy/50 uppercase tracking-[0.2em] mb-1.5">Name</label>
                <input
                  type="text" value={form.name}
                  onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                  className={inputClass} placeholder="e.g. Service Name" required
                />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >{saving ? 'Saving...' : editing ? 'Update' : 'Add'}</button>
              </div>
            </form>
          </div>
        </div>
