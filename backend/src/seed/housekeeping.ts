...
const DEEP_CLEANING_SCHEDULE: { day: string; floor: string }[] = [
...
export async function seedHousekeeping() {
  ...
  if (existingTasks < ALL_TASKS.length) {
    ...
    for (const t of newTasks) {
      await prisma.housekeepingTask.create({ data: t as any });
    }
  ...
  await prisma.housekeepingFloorTemplate.deleteMany();
  for (const tmpl of FLOOR_TEMPLATES) {
    await prisma.housekeepingFloorTemplate.create({
      data: {
        floor: tmpl.floor,
        area: tmpl.area,
  ...
  const existingDC = await prisma.housekeepingDeepCleaning.count({
  ...
  if (existingDC === 0) {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (floor) {
        await prisma.housekeepingDeepCleaning.create({
          data: {
            floor,
            date,
            startTime: '10:00',
