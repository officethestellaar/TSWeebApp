...
router.post('/table-reservation', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const { date, time, paxCount, notes } = req.body;
    const memberId = req.user?.userId;
    const affiliateId = req.user?.affiliateId;

    if (!memberId) return res.status(401).json({ message: 'User not identified' });
...
router.patch('/table-reservation/:id/process', authenticateToken, authorizeRoles('SUPER_ADMIN', 'ADMIN', 'CLUB_MANAGER', 'OPERATIONS_MANAGER', 'RESTAURANT_MANAGER'), async (req, res) => {
  try {
...
router.patch('/table-reservation/:id/cancel', authenticateToken, async (req: AuthRequest, res) => {
  try {
...
router.get('/my-table-reservations', authenticateToken, async (req: AuthRequest, res) => {
  try {
    const memberId = req.user?.userId;
    const affiliateId = req.user?.affiliateId;
...
router.post('/tables', authenticateToken, authorizeRoles('SUPER_ADMIN', 'ADMIN', 'CLUB_MANAGER'), async (req: AuthRequest, res) => {
  try {
...
router.put('/tables/:id', authenticateToken, authorizeRoles('SUPER_ADMIN', 'ADMIN', 'CLUB_MANAGER'), async (req: AuthRequest, res) => {
  try {
    const id = Number(req.params.id);
    const { number, capacity, floor } = req.body;
...
router.get('/menu', authenticateToken, async (req, res) => {
  try {
    ...
    const where: any = { isAvailable: true, department: 'RESTAURANT' };
...
router.patch('/order/:id/verify', authenticateToken, async (req, res) => {
  try {
...
router.post('/order/:id/bill', authenticateToken, async (req, res) => {
  try {
    const orderId = Number(req.params.id);
    ...
    if (order.memberId) {
      ...
      const guestNode = await prisma.member.upsert({
    ...
    await commitToLedger({
      staffId: (req as any).user?.userId || 1,
      staffName: (req as any).user?.name || 'System',
  ...
  try {
    ...
    if (status === 'READY') {
      ...
      if (recipes && recipes.length > 0) {
        ...
        for (const recipe of recipes) {
          const invItem = await prisma.inventoryItem.findUnique({ where: { id: recipe.inventoryItemId } });
...
router.patch('/order/:id/status', authenticateToken, async (req, res) => {
  try {
    const { status } = req.body;
    const order = await prisma.order.update({
