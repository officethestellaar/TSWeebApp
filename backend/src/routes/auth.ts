...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      ...
      const token = jwt.sign({ userId: user.id, role: user.role.name, name: user.name }, JWT_SECRET, { expiresIn: '8h' });
      return res.json({
        token,
    ...
    if (member && member.password && (await bcrypt.compare(password, member.password))) {
      ...
      const token = jwt.sign({ userId: member.id, role: 'MEMBER', name: member.nameAsAadhaar }, JWT_SECRET, { expiresIn: '24h' });
      return res.json({
        token,
        user: { 
          id: member.id, 
    ...
    if (familyMember && familyMember.password && (await bcrypt.compare(password, familyMember.password))) {
...
router.post('/logout', authenticateToken, async (req: AuthRequest, res) => {
  ...
  performBackup('logout').catch(err => {
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    let entity: any = await prisma.user.findUnique({ where: { email } });
    let type: 'USER' | 'MEMBER' = 'USER';

    if (!entity) {
      entity = await prisma.member.findFirst({ where: { email } });
      type = 'MEMBER';
    }
    ...
    try {
      await sendResetPasswordEmail(email, defaultPassword); 
    } catch (e) {
