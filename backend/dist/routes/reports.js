...
router.post('/import/revenue', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('SUPER_ADMIN', 'ADMIN', 'ACCOUNTANT'), upload.single('file'), async (req, res) => {
    try {
        if (!req.file)
        ...
        const results = { total: data.length, imported: 0, errors: [] };
        ...
        if (results.imported > 0) {
            ...
            cache_1.default.flushAll(); // Clear analytics cache
        }
        res.json(results);
    }
    catch (error) {
...
router.get('/stats', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const [totalMembers, todayRevenue, yesterdayRevenue, monthRevenue, lastMonthRevenue, yearRevenue, lastYearRevenue, todayMembers, yesterdayMembers, monthMembers, lastMonthMembers, yearMembers, lastYearMembers, todayInvoices, yesterdayInvoices] = await Promise.all([
            // Totals
            prisma_1.default.member.count(),
            // Revenue Aggregates
            prisma_1.default.invoice.aggregate({ where: { status: 'PAID', updatedAt: { gte: todayStart } }, _sum: { total: true } }),
            prisma_1.default.invoice.aggregate({ where: { status: 'PAID', updatedAt: { gte: yesterdayStart, lt: todayStart } }, _sum: { total: true } }),
            prisma_1.default.invoice.aggregate({ where: { status: 'PAID', updatedAt: { gte: monthStart } }, _sum: { total: true } }),
            prisma_1.default.invoice.aggregate({ where: { status: 'PAID', updatedAt: { gte: lastMonthStart, lt: monthStart } }, _sum: { total: true } }),
            prisma_1.default.invoice.aggregate({ where: { status: 'PAID', updatedAt: { gte: yearStart } }, _sum: { total: true } }),
            prisma_1.default.invoice.aggregate({ where: { status: 'PAID', updatedAt: { gte: lastYearStart, lt: yearStart } }, _sum: { total: true } }),
            // Member Growth
            prisma_1.default.member.count({ where: { createdAt: { gte: todayStart } } }),
            prisma_1.default.member.count({ where: { createdAt: { gte: yesterdayStart, lt: todayStart } } }),
            prisma_1.default.member.count({ where: { createdAt: { gte: monthStart } } }),
            prisma_1.default.member.count({ where: { createdAt: { gte: lastMonthStart, lt: monthStart } } }),
            prisma_1.default.member.count({ where: { createdAt: { gte: yearStart } } }),
            prisma_1.default.member.count({ where: { createdAt: { gte: lastYearStart, lt: yearStart } } }),
            // Detailed Invoices for Breakdown
            prisma_1.default.invoice.findMany({ where: { status: 'PAID', updatedAt: { gte: todayStart } } }),
            prisma_1.default.invoice.findMany({ where: { status: 'PAID', updatedAt: { gte: yesterdayStart, lt: todayStart } } })
        ]);
        ...
        cache_1.default.set('report_stats', responseData, 300); // 5 min cache
...
router.get('/revenue-chart', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('SUPER_ADMIN', 'ADMIN', 'ACCOUNTANT'), async (req, res) => {
    try {
        ...
        cache_1.default.set('report_revenue_chart', responseData);
        res.json(responseData);
    }
    catch (error) {
...
router.get('/membership-distribution', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('SUPER_ADMIN', 'ADMIN', 'ACCOUNTANT'), async (req, res) => {
    try {
        ...
        if (cached)
            return res.json(cached);
        const distribution = await prisma_1.default.member.groupBy({
            by: ['category'],
            _count: {
                _all: true
            }
        ...
        cache_1.default.set('report_member_dist', formattedData);
...
router.get('/gst-summary', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('SUPER_ADMIN', 'ADMIN', 'ACCOUNTANT'), async (req, res) => {
    try {
        ...
        cache_1.default.set('report_gst_summary', gstSummary);
        res.json(gstSummary);
    }
    catch (error) {
        res.status(500).json({ message: 'Internal server error' });
...
router.get('/table-turnaround', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('SUPER_ADMIN', 'ADMIN', 'ACCOUNTANT'), async (req, res) => {
    try {
        ...
        if (cached)
            return res.json(cached);
        const closedOrders = await prisma_1.default.order.findMany({
            where: { status: 'PAID' },
            include: { table: true }
        });
        const tableStats = {};
        ...
        cache_1.default.set('report_table_turnaround', responseData);
        res.json(responseData);
    }
    catch (error) {
...
router.get('/feedback/alerts', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('SUPER_ADMIN', 'ADMIN', 'CLUB_MANAGER', 'ACCOUNTANT'), async (req, res) => {
    try {
        const alerts = await prisma_1.default.feedback.findMany({
            where: {
                rating: { lte: 3 },
                isResolved: false
            },
...
// Get all feedback
router.get('/feedback', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('SUPER_ADMIN', 'ADMIN', 'CLUB_MANAGER', 'ACCOUNTANT'), async (req, res) => {
    try {
...
// Resolve feedback
router.patch('/feedback/:id/resolve', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('SUPER_ADMIN', 'ADMIN', 'CLUB_MANAGER'), async (req, res) => {
    try {
        const { id } = req.params;
        const feedback = await prisma_1.default.feedback.update({
            where: { id: Number(id) },
