...
async function autoGenerateAMCInvoices() {
    ...
    const membersDue = await prisma_1.default.member.findMany({
        where: {
            amcApplicable: true,
    ...
    for (const member of membersDue) {
        ...
        if (maxAmcs > 0) {
            ...
            const amcCount = await prisma_1.default.invoice.count({
                where: {
                    memberId: member.id,
                    department: 'AMC'
                }
...
async function generateInvoiceForMember(member, targetYear) {
    try {
...
async function checkDefaulters() {
    ...
    if (today.getMonth() === 0 && today.getDate() >= 15 || today.getMonth() > 0) {
        ...
        const candidates = await prisma_1.default.member.findMany({
            where: {
                category: { in: ['SILVER', 'GOLD'] },
                amcApplicable: true,
        ...
        for (const member of candidates) {
            ...
            const maxAmcs = tenureYears > 0 ? tenureYears - 1 : 0;
            if (maxAmcs > 0) {
                const paidAmcs = await prisma_1.default.invoice.count({
                    where: {
                        memberId: member.id,
                        department: 'AMC',
                        status: 'PAID'
...
async function checkHousekeepingReminders() {
    ...
    const pendingInstances = await prisma_1.default.housekeepingTaskInstance.findMany({
    ...
    for (const instance of pendingInstances) {
        ...
        if (diffMin >= -15 && diffMin < 0) {
            (0, socket_1.emitEvent)('housekeeping_reminder', {
                type: 'UPCOMING',
        ...
        if (diffMin >= 30 && instance.status === 'PENDING') {
            await prisma_1.default.housekeepingTaskInstance.update({
